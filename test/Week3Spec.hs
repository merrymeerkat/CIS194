...
spec =
    describe "Week 3" $ do
        ...
        describe "evalE" $ do
            it "should evaluate an expression given a state" $ do
        ...
        describe "evalSimple" $ do
            it "should evaluate a desugared statement" $ do
                (let s = evalSimple empty (DAssign "A" (Val 10)) in s "A" == 10) `shouldBe` True
                (let s1 = evalSimple empty (DAssign "B" (Val 1)) in s1 "B" == 10) `shouldBe` False
        describe "run" $ do
            it "should run a programme that has not already been desugared" $ do
